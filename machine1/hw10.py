...
# female + pclass 1~2 > survival rates high
test1_data['Survived'] = np.where(
    (test1_data['Sex'] == 'female') | (test1_data['Pclass'] < 3), 1, 0
)

# 결측치 승객은 Survived를 NaN으로 설정
test1_data.loc[test1_data.isnull().any(axis=1), 'Survived'] = np.nan

# 결과 확인
print(test1_data[['PassengerId', 'Survived']])